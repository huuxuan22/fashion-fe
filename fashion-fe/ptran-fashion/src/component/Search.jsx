...
const Search = () => {
  ...
  const loadProducts = async () => {
    if (searchTerm.trim() !== "") {
      await productService.searchProduct({page: page, size: itemsPerPage, value: searchTerm,token:token}).then((data) => {
        setProducts(data.data.content);
      })
      await productService.getAllTotalPage({value: searchTerm,token:token}).then((data) => {
            ...
            <>
              <Grid container spacing={3} sx={{cursor: "pointer"}}>
                {products.map((product) => (
                  <Grid item xs={12} sm={6} md={4} lg={2} key={product.productId} onClick={() => handleProductDetail(product.productId)}>
